...
import { Opportunity, Course, EnrollmentState, UserProfile, Category } from '../types';
...
export default function CabinetView({
  ...
  const monthsKk = ['қаң', 'ақп', 'нау', 'сәу', 'мам', 'мау', 'шіл', 'там', 'қыр', 'қаз', 'қара', 'жел'];

  return (
    <div id="student-cabinet" className="space-y-8 font-sans">
      {/* 1. Core Profile Header Card */}
      <div className="bg-white border border-slate-200 rounded-xl p-6 sm:p-8 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 text-left">
        <div className="flex items-start gap-4 w-full sm:w-auto">
          ...
          {isEditing ? (
            <form onSubmit={handleSave} className="space-y-3 w-full max-w-sm font-sans text-left">
              <div className="space-y-1">
                <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">
                  {language === 'en' ? 'Student Full Name' : language === 'kk' ? 'Оқушының толық аты-жөні' : 'ФИО Ученика'}
                </label>
                <input
                  type="text"
                  required
              ...
              <div className="space-y-1">
                <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">{translate('authEmail', language)}</label>
                <input
                  type="email"
                  required
                  value={editEmail}
              ...
              <div className="space-y-1 text-left">
                <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">{translate('authGrade', language)}</label>
                <select
                  value={editGrade}
                ...
                >
                  {['8', '9', '10', '11', '12'].map((g) => (
                    <option key={g} value={g}>{translateGrade(g, language)}</option>
                  ))}
                </select>
              </div>
              <div className="flex gap-2 pt-1 font-sans text-xs">
                ...
                >
                  {translate('cabSaveProfile', language)}
                </button>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="px-3.5 py-1.5 border border-slate-250 hover:bg-slate-50 text-slate-655 hover:text-slate-800 rounded font-bold transition-colors cursor-pointer uppercase text-[10px] tracking-wide"
                >
                  {language === 'en' ? 'Cancel' : language === 'kk' ? 'Бас тарту' : 'Отмена'}
                </button>
              </div>
            </form>
          ) : (
            <div className="space-y-1 flex-1 text-left">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="bg-blue-600 text-white font-extrabold text-[10px] uppercase px-2.5 py-0.5 rounded border border-blue-500 select-none">
                  {translateGrade(userProfile.grade, language)}
                </span>
              </div>
              ...
              <div className="pt-1.5">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 text-blue-600 hover:text-blue-800 text-[11px] font-extrabold uppercase tracking-wide transition-all cursor-pointer bg-blue-5/10 hover:bg-blue-50 px-2.5 py-1 rounded"
                >
                  <Edit3 className="w-3 h-3" />
                  {translate('cabEditProfile', language)}
                </button>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 pt-2">
                {userProfile.interests.map((interest) => (
                  ...
                  >
                    {translateCategory(interest as Category, language)}
                  </span>
                ))}
              </div>
            </div>
        ...
        >
          ...
          {translate('cabResetPref', language)}
        </button>
      </div>
      ...
      <section id="cabinet-goals-checklist" className="bg-white border border-slate-200 p-6 rounded-xl space-y-4 shadow-sm text-left">
        <h3 className="font-extrabold text-slate-800 text-xs flex items-center gap-2 uppercase tracking-wide">
          ...
          {language === 'en' ? 'Your Educational Goals' : language === 'kk' ? 'Сіздің білім беру мақсаттарыңыз' : 'Ваши образовательные цели'}
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 text-xs text-slate-705 font-light">
          {userProfile.goals.map((g, index) => (
            <div key={index} className="flex items-start gap-2.5 bg-slate-50 p-3 rounded border border-slate-200">
              ...
              <p className="leading-relaxed text-slate-755 font-normal">{g}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start text-left">
        {/* Left Column (Courses and saved stuff) */}
        <div className="lg:col-span-8 space-y-8">
          {/* Active Courses Progress Tracker */}
          <div className="space-y-4">
            <h3 className="text-base font-extrabold text-slate-800 flex items-center gap-2 uppercase tracking-wide">
              <BookOpen className="w-5 h-5 text-blue-600" />
              {translate('cabMyCourses', language)} ({enrolledCoursesList.length})
            </h3>
            ...
            {enrolledCoursesList.length === 0 ? (
              <div className="bg-white border border-slate-200 rounded-xl p-8 text-center space-y-3.5 shadow-sm">
                ...
                <p className="text-slate-550 text-xs">
                  {translate('cabNoCourses', language)}
                </p>
                <button
                ...
                >
                  {language === 'en' ? 'Explore Courses' : language === 'kk' ? 'Курстар каталогына өту' : 'Перейти к каталогу курсов'}
                </button>
              </div>
            ) : (
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-1">
                          <div className="flex justify-between text-[10px] text-slate-450 font-bold font-mono">
                            <span>{language === 'en' ? 'Lessons Completed' : language === 'kk' ? 'Аяқталған сабақтар' : 'Пройдено уроков'}</span>
                        ...
                        >
                          {translate('cabContinueCourse', language)}
                        </button>
                      </div>
                    </div>
                  );
          ...
          <div className="space-y-4">
            <h3 className="text-base font-extrabold text-slate-800 flex items-center gap-2 uppercase tracking-wide">
              ...
              {translate('cabSavedOpportunities', language)} ({savedOpsList.length})
            </h3>
            ...
            {savedOpsList.length === 0 ? (
              <div className="bg-white border border-slate-200 rounded-xl p-8 text-center space-y-3.5 shadow-sm">
                <span className="text-3xl block"></span>
                <p className="text-slate-550 text-xs">
                  {translate('cabNoSaved', language)}
                </p>
                <button
                  onClick={() => setCurrentTab('catalog')}
                  className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition-all cursor-pointer shadow-md animate-bounce"
                >
                  {translate('cabGoToCatalog', language)}
                </button>
              </div>
            ) : (
              <div className="space-y-3.5">
                {savedOpsList.map((opp) => (
                  ...
                  >
                    <div className="space-y-1 flex-1 text-left">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-[10px] font-bold px-2 py-0.5 uppercase bg-slate-150 text-slate-800 border border-slate-250 rounded animate-none">
                          {translateCategory(opp.category, language)}
                        </span>
                        <span className="text-[10px] text-slate-400 font-mono font-medium">
                          {translate('cardFormat', language)} {translateFormat(opp.format, language)}
                        </span>
                      </div>
                      <h4 className="font-extrabold text-slate-800 text-sm leading-snug">{opp.title}</h4>
                      <p className="text-[11px] text-slate-450">{translate('cardOrg', language)} {opp.organization}</p>
                    </div>

                    <div className="flex sm:flex-col items-end gap-2.5 justify-between select-none shrink-0 text-right">
                      <div>
                        <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">{language === 'en' ? 'Deadline' : language === 'kk' ? 'Соңғы мерзімі' : 'Дедлайн подачи'}</p>
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="px-3 py-1.5 border border-slate-200 text-slate-500 hover:text-red-550 hover:bg-rose-50 rounded-lg text-xs font-bold transition-colors cursor-pointer"
                          title={language === 'en' ? 'Remove' : language === 'kk' ? 'Алып тастау' : 'Убрать'}
                        >
                          {language === 'en' ? '✕ Remove' : language === 'kk' ? '✕ Өшіру' : '✕ Убрать'}
                        </button>
                        <button
                          onClick={() => setCurrentTab('catalog')}
                          className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition-all cursor-pointer shadow-sm font-semibold"
                        >
        ...
        <aside className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white border border-slate-200 p-6 rounded-xl space-y-4 shadow-sm font-sans text-left">
            <h3 className="font-extrabold text-slate-850 text-xs flex items-center gap-2 uppercase tracking-wide">
              <Clock className="w-5 h-5 text-rose-600" />
              {language === 'en' ? 'Deadlines Radar (2026)' : language === 'kk' ? 'Дедлайн радары (2026)' : 'Радар дедлайнов (2026 год)'}
            </h3>
            
            {savedOpsList.length === 0 ? (
              <p className="text-slate-455 text-xs leading-relaxed font-light">
                {translate('cabSavedOppsDesc', language)}
              </p>
            ) : (
              <div className="space-y-3">
                {savedOpsList
                  ...
                  .slice(0, 3)
                  .map((opp) => {
                    const mIndex = parseInt(opp.deadline.split('-')[1]) - 1;
                    const mLabel = language === 'en' ? monthsEn[mIndex] : language === 'kk' ? monthsKk[mIndex] : monthsRu[mIndex];
                    return (
          ...
          <div className="bg-slate-900 border border-slate-850 text-white p-6 rounded-xl space-y-4 shadow-md relative overflow-hidden text-left">
            ...
            <div className="flex items-center gap-2 text-blue-300 font-bold text-[10px] uppercase tracking-wider font-mono">
              ...
              <span>{language === 'en' ? 'AI Recommendation' : language === 'kk' ? 'ИИ ұсынысы' : 'Рекомендотация ИИ от Mentoria'}</span>
            </div>

            <h3 className="font-extrabold text-sm sm:text-base uppercase tracking-tight">
              {language === 'en' ? 'How to boost your CV this month?' : language === 'kk' ? 'Осы айда түйіндемені қалай күшейте аласыз?' : 'Как усилить резюме в этом месяце?'}
            </h3>
            
            {portfolioRecommendations.length > 0 ? (
              <div className="space-y-3 text-xs leading-relaxed text-slate-300 font-light pt-1 text-left">
                <p>
                  {language === 'en' ? (
                    <>Since you are interested in <strong>{translateCategory(userProfile.interests[0] as Category, language)}</strong>, we suggest checking these options:</>
                  ) : language === 'kk' ? (
                    <>Сіз <strong>{translateCategory(userProfile.interests[0] as Category, language)}</strong> саласына қызыққандықтан, келесілерді көруді ұсынамыз:</>
                  ) : (
                    <>Поскольку вы интересуетесь направлением <strong>{translateCategory(userProfile.interests[0] as Category, language)}</strong>, мы рекомендуем вам обратить пристальное внимание на следующие неактивированные опции:</>
                  )}
                </p>
                <div className="space-y-2">
                  {portfolioRecommendations.map((o) => (
                    ...
                    >
                      ...
                      <p className="text-[10px] text-blue-300 truncate font-mono">{translate('cardDeadline', language)} {o.deadline}</p>
                    </div>
                  ))}
                </div>
              </div>
            ) : (
              <p className="text-slate-300 text-xs font-light leading-relaxed text-left">
                {language === 'en' ? 'Complete at least one interactive quiz 100% correct today to unlock potential research opportunities!' : language === 'kk' ? 'Сабақ тапсырмаларын орындап, жаңа мүмкіндіктерді ашу үшін бүгін квиздерді сәтті аяқтаңыз!' : 'Пройдите по крайней мере 1 асинхронный тест на 100% сегодня, чтобы автоматически открыть доступ к скрытым исследовательским программам!'}
              </p>
            )}
          </div>
        </aside>
